...
- [visualizer.py](src/sortshort/visualizer.py) — pygame-ce app. Owns no sorting logic: each frame it pulls `speed` steps from the active generator, remembers the last `Highlight`, and redraws bars. State machine `RUNNING → VERIFYING → DONE` (the verify sweep lights the array green when the generator is exhausted). Lazily imported by `__main__` so `--help` needs no display.
- [__main__.py](src/sortshort/__main__.py) — argparse CLI (`--size/--fps/--algo/--seed`) and the `sortshort` entry point.
